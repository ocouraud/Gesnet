...
type
  TFrameTableTarif = class(TFrame)
    Panel2: TPanel;
    BtnFermer: TBitBtn;
    BtnAide: TBitBtn;
    JvDBGridTarif: TJvDBGrid;
    BtnSupprimer: TBitBtn;
    DSTarif: TDataSource;
    procedure JvDBGridTarifKeyPress(Sender: TObject; var Key: Char);
...
procedure TFrameTableTarif.BtnAideClick(Sender: TObject);
begin
  ...
  FormAide.AfficherAide('tarif_liste.html');

end;

procedure TFrameTableTarif.BtnFermerClick(Sender: TObject);
var
...
procedure TFrameTableTarif.BtnSupprimerClick(Sender: TObject);
begin
  // 1. Vérifie si le DataSet n'est pas vide et contient des données
  if not DMGesCloud.ReqTarif.IsEmpty then
  begin
    // 2. Demande confirmation à l'utilisateur
    if MessageDlg('Voulez-vous vraiment supprimer cette ligne ?',
                  mtConfirmation, [mbYes, mbNo], 0) = mrYes then
    begin
      DMGesCloud.ReqTarif.Delete;
    end;
  end;
end;

constructor TFrameTableTarif.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
  DMGesCloud.ReqTarif.Open;
end;

procedure TFrameTableTarif.JvDBGridTarifKeyPress(Sender: TObject; var Key: Char);
begin
