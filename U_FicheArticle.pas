...
begin
  ...
  ACodFac := FDQueryLigvteaa.FieldByName('CODFAC').AsInteger;
  // 1. Activer la requête SQL contenant les données de la facture
  FDQueryFacture.ParamByName('CODFAC').AsInteger := ACodFac;
  FDQueryFacture.Open;
  ...
  frxReportFacture.Variables.AddVariable('Globales','VarTotalAlpha',QuotedStr('Facture arrêtée à la somme de : ' +
    DMGesCloud.MontantenLettres(FDQueryFacture.FieldByName('MT_TTC').AsInteger) + ' Francs CFP.'));
  if FDQueryFacture.FieldByName('TYPE_').AsString='F' then
  ...
  DM_Olivier.FDQueryRepres.ParamByName('CODREP').AsInteger:= FDQueryFacture.FieldByName('CODREP').AsInteger;
  ...
  FDQueryTVAFacture.ParamByName('CODFAC').AsInteger := FDQueryFacture.FieldByName('CODFAC').AsInteger;
  FDQueryTVAFacture.Open;

  FDQueryReglements.ParamByName('CODFAC').AsInteger := FDQueryFacture.FieldByName('CODFAC').AsInteger;
