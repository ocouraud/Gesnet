...
type
  TFrameTableDepart = class(TFrame)
    Panel2: TPanel;
    BtnFermer: TBitBtn;
    BtnAide: TBitBtn;
    JvDBGridDepart: TJvDBGrid;
    BtnSupprimer: TBitBtn;
    DSDepart: TDataSource;
    procedure JvDBGridDepartKeyPress(Sender: TObject; var Key: Char);
...
procedure TFrameTableDepart.BtnAideClick(Sender: TObject);
begin
  ...
  FormAide.AfficherAide('depart_liste.html');

end;

procedure TFrameTableDepart.BtnFermerClick(Sender: TObject);
var
...
procedure TFrameTableDepart.BtnSupprimerClick(Sender: TObject);
begin
  // 1. Vérifie si le DataSet n'est pas vide et contient des données
  if not DM_Olivier.FDQueryDepart.IsEmpty then
  begin
    // 2. Demande confirmation à l'utilisateur
    if MessageDlg('Voulez-vous vraiment supprimer cette ligne ?',
                  mtConfirmation, [mbYes, mbNo], 0) = mrYes then
    begin
      DM_Olivier.FDQueryDepart.Delete;
    end;
  end;
end;

constructor TFrameTableDepart.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
  DM_Olivier.FDQueryDepart.Open;
end;

procedure TFrameTableDepart.JvDBGridDepartKeyPress(Sender: TObject; var Key: Char);
begin
