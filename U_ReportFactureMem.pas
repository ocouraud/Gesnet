...
type
  TFormFactureMemPrint = class(TForm)
    RLReport1: TRLReport;
    ...
    JvShape5: TJvShape;
    JvShape6: TJvShape;
    FDMemTableEntvte: TFDMemTable;
    FDMemTableLigvte: TFDMemTable;
    FDMemTableRegl: TFDMemTable;
  ...
  public
    { Public declarations }
    procedure ImprimerFacture(SourceEntvte, SourceLigvte, SourceRegla: TFDDataSet);
  end;
...
procedure TFormFactureMemPrint.ImprimerFacture(SourceEntvte, SourceLigvte, SourceRegla: TFDDataSet);
begin
  // Copie instantanée des blocs de données source vers les mémoires de l'état
  FDMemTableEntvte.Data := SourceEntvte.Data;
  FDMemTableLigvte.Data := SourceLigvte.Data;
  FDMemTableRegl.Data  := SourceRegla.Data;
...
procedure TFormFactureMemPrint.RLReport1BeforePrint(Sender: TObject; var PrintIt: Boolean);
begin
  // 1. On ouvre la table de configuration de l'entreprise si elle est fermée
  if not DMGesCloud.FDQueryRepres.Active then
    DMGesCloud.FDQueryRepres.ParamByName('CODREP').AsInteger:=DSEntvteaa.DataSet.FieldByName('CODREP').AsInteger;
    DMGesCloud.FDQueryRepres.Open;

  if not FDQueryClient.Active then
    FDQueryClient.ParamByName('CODCLI').AsInteger:=DSEntvteaa.DataSet.FieldByName('CODCLI').AsInteger;
    FDQueryClient.Open;
...
procedure TFormFactureMemPrint.RLBandDetailLignesBeforePrint(Sender: TObject;
  var PrintIt: Boolean);
begin
...
procedure TFormFactureMemPrint.RLBandHeaderBeforePrint(Sender: TObject; var PrintIt: Boolean);
var
  CheminLogo: string;
begin
...
procedure TFormFactureMemPrint.RLBandSummaryBeforePrint(Sender: TObject; var PrintIt: Boolean);
var
  i: Integer;
  QueryEnt: TDataSet;
  QueryPar: TDataSet;
...
// 1. GESTION DYNAMIQUE DU BLOC TVA (Un seul texte combiné)
  ...
  FDMemTableRegl.First;
  i := 1;
  while (not FDMemTableRegl.Eof) and (i <= 4) do
  begin
    case i of
      1: begin
           RLLblRegL1_Lib.Caption := FDMemTableRegl.FieldByName('LIBELLE').AsString;
           RLLblRegL1_Mnt.Caption := FormatFloat('#,##0', FDMemTableRegl.FieldByName('MONTANT').AsCurrency);
           RLLblRegL1_Ech.Caption := FDMemTableRegl.FieldByName('DATE_ECH').AsString;
         end;
      2: begin
           RLLblRegL2_Lib.Caption := FDMemTableRegl.FieldByName('LIBELLE').AsString;
           RLLblRegL2_Mnt.Caption := FormatFloat('#,##0', FDMemTableRegl.FieldByName('MONTANT').AsCurrency);
           RLLblRegL2_Ech.Caption := FDMemTableRegl.FieldByName('DATE_ECH').AsString;
         end;
      3: begin
           RLLblRegL3_Lib.Caption := FDMemTableRegl.FieldByName('LIBELLE').AsString;
           RLLblRegL3_Mnt.Caption := FormatFloat('#,##0', FDMemTableRegl.FieldByName('MONTANT').AsCurrency);
           RLLblRegL3_Ech.Caption := FDMemTableRegl.FieldByName('DATE_ECH').AsString;
         end;
      4: begin
           RLLblRegL4_Lib.Caption := FDMemTableRegl.FieldByName('LIBELLE').AsString;
           RLLblRegL4_Mnt.Caption := FormatFloat('#,##0', FDMemTableRegl.FieldByName('MONTANT').AsCurrency);
           RLLblRegL4_Ech.Caption := FDMemTableRegl.FieldByName('DATE_ECH').AsString;
         end;
    end;
    Inc(i);
    FDMemTableRegl.Next;
